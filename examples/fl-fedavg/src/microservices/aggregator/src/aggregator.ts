...
var initTimeout: ReturnType<typeof setTimeout>;
var trainTimeout: ReturnType<typeof setTimeout>;
var roundTimeout: ReturnType<typeof setTimeout>;

const X_KLEINT_SESSION_SENDER = "X-Kleint-Session-Sender";
const X_KLEINT_SESSION_DESTINATION = "X-Kleint-Session-Destination";
var fetchHeaders: HeadersInit & FetchHeaders = {
  "Content-Type": "application/json",
  [X_KLEINT_SESSION_DESTINATION]: "",
};

var initTrainBody: RunTrainBody = {
  round: 0,
  numEpochs: undefined,
...
var initializedSessions: { [key: string]: string[] } = {
  client: [],
  evaluator: [],
};
var totalNumRounds;
var currentRound = 0;
var aggregatedWeights: Weights = {};
var resetWeigthts = true;
var aggregationPromises: Promise<number>[] = [];
var aggregationPromisesResolve: PromisesResolves = {};
var evaluationPromise: Promise<void> = null;
var clientCount = 0;
...
export async function start(
  ...
  initFromParams(JSON.parse(body) as StartParams);
...
export async function aggregate(
  ...
  let localWeights = body.weights,
    localLength = body.length,
    session = getHeader(headers, X_KLEINT_SESSION_SENDER);

  if (!trainTimeout) handleTrainTimeout();
...
async function _aggregate(localWeights: Weights, localLength: number) {
  ...
  if (clientCount == 0) {
    for (let i = 0; i < layers.length; i++) {
      ...
      aggregatedWeights[layerName] = scalarProduct(
        localWeights[layerName],
        localLength
      );
    }
  } else {
    for (let i = 0; i < layers.length; i++) {
      let layerName = layers[i];
      aggregatedWeights[layerName] = addArrays(
        aggregatedWeights[layerName],
        scalarProduct(localWeights[layerName], localLength)
      );
    }
  }
  clientCount += 1;
  console.log(
    `[AGGREGATOR] Aggregated client ${clientCount}/${aggregationPromises.length}`
  );
  lock.unlock();
}

async function initNewSessions(
  sessions: string[],
  destination = "client",
  sync = true
) {
  let notInitializedSessions = sessions.filter(
      (x) => !initializedSessions[destination].includes(x)
    ),
    newSessions = [];
  ...
  initializedSessions[destination] = sessions.filter((x) =>
    newInitializedSessions.includes(x)
  );
}
...
async function initSession(
  ...
  let bodyToSend: InitSessionBody = {
  ...
  let initClientPromises: Promise<string>[] = [],
    headers: HeadersInit & FetchHeaders = { ...fetchHeaders },
    fetchParams = {
      method: "POST",
      headers,
  ...
  for (let i = 0; i < sessions.length; i++) {
    ...
    let promisesWithTimeout: Promise<string> = initWithTimeout(
      response,
      sessions[i]
    );
...
async function getActiveSessions(
  sessionType: string = "client"
): Promise<string[]> {
  try {
    let response = await fetch(
      "http://kleint-gateway:11355/getActiveSessions/" + sessionType
    );
...
async function aggregateRound() {
  let total_clients_lengths = await Promise.all(aggregationPromises),
    totalLength = total_clients_lengths.reduce((a, b) => a + b, 0),
    layers = Object.keys(aggregatedWeights),
    layerName: string;
  ...
  if (totalLength > 0) {
    ...
    runEvalStep();
    console.log(
      `[AGGREGATOR] Communication round ${currentRound}/${totalNumRounds} completed!`
    );
...
async function startRound(destination = "client", sync = true) {
  ...
  GUILog(
    `Initializing client sessions for round ${
      currentRound + 1
    }/${totalNumRounds}!`
  );
  const clients = await getActiveSessions();
  ...
  let minParticipants = Math.min(
      initializedSessions.client.length,
      startParams.clientNums
    ),
    numPartecipants = Math.ceil(minParticipants * startParams.fraction);
  if (numPartecipants < startParams.minClientNums) {
    ...
    return setTimeout(
      () => startRound(destination, sync),
      startParams.roundTimeout
    );
  } else if (numPartecipants < startParams.clientNums) {
    console.warn(
      "[AGGREGATOR] Your participants are less than the expected number - " +
        minParticipants +
        "/" +
        startParams.clientNums
    );
  ...
  currentRound++;
  console.log(
    `[AGGREGATOR] Start round ${currentRound}/${totalNumRounds} with ${numPartecipants} clients!`
  );
  GUILog(`Starting round`);

  let currentRoundClients = pickRandomClients(
    initializedSessions.client,
    numPartecipants
  );

  let excludedRoundClients = initializedSessions.client.filter(
    (x) => !currentRoundClients.includes(x)
  );
...
function runTrainStep(
  currentRoundClients: string[],
  destination = "client"
): Promise<Response>[] {
  aggregationPromises = [];
  aggregationPromisesResolve = {};

  let newAggregatedParams =
    Object.keys(aggregatedWeights).length > 0 ? aggregatedWeights : null;
  let bodyToSend: RunTrainBody = {
      ...initTrainBody,
      round: currentRound,
      modelWeights: newAggregatedParams,
    },
    jsonBody = JSON.stringify(bodyToSend),
    fetchResponses: Promise<Response>[] = [],

    headers: HeadersInit & FetchHeaders = { ...fetchHeaders, ['X-Kleint-Timeout']: `${5000}`},
    fetchParams = {
      method: "POST",
      headers,
      body: jsonBody,
...
async function sendParams(
  ...
  let clientPromises: Promise<Response>[] = [],
    headers: HeadersInit & FetchHeaders = { ...fetchHeaders },
...
async function runEvalStep() {
  ...
  let newAggregatedParams =
    Object.keys(aggregatedWeights).length > 0 ? aggregatedWeights : null;
  let bodyToSend = { round: currentRound, modelWeights: newAggregatedParams },
    jsonBody = JSON.stringify(bodyToSend),
    fetchResponses: Promise<Response>[] = [],
    headers: HeadersInit & FetchHeaders = { ...fetchHeaders },
    fetchParams = {
...
function pickRandomClients(clientSessions: string[], numPartecipants: number) {
  let randomSessions: string[] = [];
...
function resolvePendingPromises(promises: PromisesResolves) {
...
function scalarProduct(array: number[], scalar: number): number[] {
  let result = [];
...
function checkUndefinedValues(obj: Object): boolean {
  console.log(obj);
...
async function checkFetchResponses(
  message = "Request sent correctly",
  responses: Response[]
): Promise<FetchResponse> {
  for (let i = 0; i < responses.length; i++) {
    if (!responses[i].ok) {
      let body = await responses[i].json(),
        message = `${responses[i].status} - 
...
function initWithTimeout(promise, session): Promise<string> {
...
const GUILog = async (message: string) => {
...
function initFromParams(params: StartParams) {
  startParams = {
